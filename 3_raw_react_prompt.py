...
def get_tool_descriptions(tools_dict):
    ...
    for tool_name, tool_function in tools_dict.items():
        ...
        # agent introduced as part of the initiative to be 
        original_function = getattr(tool_function, "__wrapped__", tool_function)
        signature = inspect.signature(original_function)
        # result  is dependent of observation + reasoning obtained from the llm
        docstring = inspect.getdoc(tool_function) or ""
        descriptions.append(f"{tool_name}{signature} - {docstring}")
        # format of the output is joined descriptions
    return "\n".join(descriptions)
...
# --- Agent Loop ---
# Agent lookup is part of the directio 
...
def run_agent(question: str):
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        # Another variant of the tools based llm
        # When the base model is changed we need to changee the functions and 
        # this is going to create a list
